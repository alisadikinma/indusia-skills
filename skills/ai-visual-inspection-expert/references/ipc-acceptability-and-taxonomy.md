...
## 6. AUDIT TRAIL & DRIFT (project-relevant)

### Model Traceability and Explainability
*   **Heatmap on every reject:** a rejection should be traceable via **Grad-CAM / activation heatmap** so a human can verify *why* the model called a defect. This is the loop behind the project's correction-mode and the two HITL gates — an unexplainable reject is not auditable.

### Drift & re-validation
*   **Re-validate on board revision or component-vendor change.** Those shift the "good" distribution and silently raise the escape rate. Audit the per-class confusion for drift; if accuracy degrades against the held-out baseline, route back to correction/retrain — do not let a drifting model keep promoting.

> Enterprise cloud-security/MLOps tooling (ISO-27001, VPC, AWS KMS, IAM,
> MLflow, SageMaker Model Monitor) is **out of scope** for this product and was
> removed — the editor stores assets on local fs and drives the sibling service
> over HTTP. Keep the principle (audit + drift watch), drop the cloud stack.