...
## 0. Product reality (2026) — read this first, it overrides generic defaults

This product is **not** a YOLO-heavy defect-labeled AOI line. It is the opposite
corner, and the advice has to match it. The full grounded doctrine + cited numbers
live in [references/golden-sample-deployment-reality.md](references/golden-sample-deployment-reality.md) —
the four bullets below are the load-bearing summary:

- **Template-match-first.** `template_match` carries the bulk (presence/absence,
  polarity, gross alignment) and **deploys with no training** — it is ~90% of a
  well-fixtured line (F1 ≈ 0.982 under rigid fixture + stable light). Reach for
  `anomalib`/`yolo` **per component, only where template matching demonstrably
  fails** (specular parts, registration error, benign part-to-part variation) —
  never wholesale. Supervised YOLO is the **exception**, not the default.
- **Few good boards ≠ enough for anomaly.** Anomaly-on-good needs **10–100
  *distinct physical good boards*** (PatchCore 10–100, few-shot 1–5). The LS-first
  import gives N *frames of one board* (intra-board variance) — that is **not** N
  good boards (inter-board variance) and does not defeat golden-sample selection
  bias. Flag it at Gate 1. Don't pool different camera viewpoints into one good set.
- **Golden-only data is honestly UNSCORED.** With no labeled defect test set,
  supervised **mAP/F1/recall cannot be computed** — `scored: false` is *correct*.
  Do **not** quote "mAP ≥ 0.80" as the promote line here (see §5). Qualify instead
  with held-out-good overkill PPM, Attribute Agreement κ, and the Rule of Three.
- **Gate 2 promotes by process, not by a fabricated metric.** Shadow 7–14 d →
  pilot 50–500 u (escape 0%, overkill < 500 PPM) → supervisor override with an
  audited trail. `template_match` has no model-runtime version risk; an `anomalib`
  `model.pt` (anomalib 2.x) needs a runtime-version gate before it can deploy.

---

## 1. How to operate

1. **Template-match-first, then escalate by signature.** For each component ask:
   fixed fixture + stable look + presence/polarity/alignment → `template_match`
   (no training). Benign look-variation/specular **and** ≥10–100 good boards →
   `anomalib`. Repeating defect with **real labels** → `yolo`. Text/code → `ocr`/
   `barcode`. Height (tombstone, lifted lead) or hidden (BGA) → **out of 2D scope,
   flag it.** Model choice is a function of *defect morphology* + *data you actually
   have*, not fashion.
...
3. **Quantify — but only metrics you can actually compute.** Give the number
   (template F1 / AUROC / overkill PPM / κ), the latency budget, and the
   training-data regime. In the golden-only flow, supervised mAP/F1 are *uncomputable*
   — say so and use the §5 measures. Cite the reference files; don't invent figures.
...
| Need | Read |
|---|---|
| **THIS PRODUCT: template-match sufficiency + thresholds, min good-board counts + selection bias, eval-without-defects (overkill PPM / AAA-Kappa / Rule of Three), shadow→pilot→override governance** | [references/golden-sample-deployment-reality.md](references/golden-sample-deployment-reality.md) |
...
| Model rationale, supervised-vs-anomaly, 2D-scope/3D-X-ray honest limits, training strategy | [references/model-selection-per-defect.md](references/model-selection-per-defect.md) |
...
> Provenance: the reference files are NotebookLM syntheses. The four generic refs draw
> on an earlier multi-pass synthesis (model-per-defect, IPC, EMS taxonomy, solder-joint,
> anomaly, OCR/fiducial, imaging, MI-THT, AI-AOI ops); cloud-MLOps/3D/X-ray content that
> doesn't apply to this single-camera 2D golden-sample editor was pruned (2026-06-21).
> The **golden-sample-deployment-reality** ref is a fresh **48-source** deep-research pass
> (2026-06-21) and is the authority where it conflicts with the generic refs. Grounded
> summaries — when a cell says "Not in source", say so; do not invent. Field-playbook
> YOLO11 benchmarks are paper-specific — reference points, not hard specs.
...
|---|---|---|---|---|---|
| Missing component | `missing_component` | template NCC/SSIM vs golden ROI; CM-YOLO if labeled | 2D, +3D helps | **`template_match`** → `anomalib` → `yolo` | no-training → anomaly good-only → supervised |
| Wrong component | `wrong_value` | YOLOv5/v8 + OCR cross-check vs BOM | 2D + OCR | `template_match` (presence) + `ocr`/`yolo` (identity) | no-training → supervised |
| Wrong value / marking | `wrong_value` | OCR/OCV engine (deep text) | OCR | `ocr` | supervised many |
| Polarity / orientation reversal | `polarity_flip`, `orientation` | template SSIM on asymmetric marker; YOLOv8+CBAM if it fails | 2D, +OCV | **`template_match`** → `yolo` (+`ocr`) | no-training → supervised · few-shot |
| Misalignment / shift | `misalignment` | template NCC vs golden ROI; CM-YOLO if it drifts | 2D, +3D | **`template_match`** → `yolo` | no-training → supervised · few-shot |
...
attach per criterion. Where a cell shows `A → B → C`, attach the **leftmost** that
holds for that component and only escalate when it demonstrably fails (§0 template-
match-first, §golden-sample-deployment-reality §1) — `template_match` ships with no
training and carries ~9/10 components on a fixed-fixture line. Height-class defects
(tombstone, billboard, lifted lead) are the honest limit of single-camera 2D — flag
them and recommend anomaly-on-good or a depth source rather than over-promising a 2D YOLO.
...
- **Few-shot / foundation fine-tune** for components with little history; **anomaly
  good-only** for the rare tail — but only with **≥10–100 distinct good boards**, not
  N frames of one board (selection bias — see the golden-sample-deployment-reality ref §2).
- **Eval thresholds — know WHICH regime you're in:**
  - *With a labeled defect test set* (v1.5): the FE `api/eval.ts` line applies —
    mAP **0.80**, F1 macro **0.80**, per-component F1 **0.70**; a component under
    per-comp F1 0.70 routes back to correction, it does not promote.
  - *Golden-only / template-match / anomaly-on-good (the v1 reality):* there is **no
    labeled defect test set**, so **mAP/F1/recall are uncomputable** and eval is
    honestly `scored: false`. Do **not** quote the 0.80 line here. Qualify instead with
    **held-out-good overkill < 500 PPM**, **Attribute Agreement κ ≥ 0.90** (20–30 parts
    × 2–3 appraisers × 2–3 trials), and a **Rule-of-Three** escape bound (3/n; 3,000
    clean inspections → <1000 PPM @95%). Full method: golden-sample-deployment-reality ref §3.
